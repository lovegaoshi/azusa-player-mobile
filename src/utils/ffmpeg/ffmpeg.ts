...
export const ffmpegToMP3 = async ({
  ...
  if (song) {
    ...
    const command = `-i '${fspath}' -vn -ab 256k -id3v2_version 3 -metadata title='${song.name.replaceAll("'", '')}' -metadata artist='${song.singer}' -metadata album='${song.album?.replaceAll("'", '') ?? ''}' ${fspath}.mp3`;
